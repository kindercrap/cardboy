...
For a faster refresh of saved cards, open **My Cards → Update Queue**, replace any older CardBoy bookmark with the current one, and choose **Start Quick Update**. CardBoy opens one Yuyu-tei tab. Click the bookmark once on each card page; the exact source-URL match is saved automatically, price history and notifications are updated, and that same tab advances to the next queued card. Older bookmarks also trigger automatic saving on queue-marked pages, although replacing the bookmark provides the cleaner one-tab loop. A normal Yuyu-tei tab still uses the safer review form, and a new card is never added automatically.
